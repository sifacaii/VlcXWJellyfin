...
Java_org_videolan_libvlc_MediaPlayer_nativeSetVideoFormat(JNIEnv *env,jobject thiz,
                                                          jstring chroma,jint width,jint height,jint pitch)
{
    vlcjni_object *p_obj = VLCJniObject_getInstance(env,thiz);
    ...
    const char *formatStr = (*env)->GetStringUTFChars(env,chroma,NULL);

    libvlc_video_set_format(p_obj->u.p_mp,formatStr,width,height,pitch);

    (*env)->ReleaseStringUTFChars(env,chroma,formatStr);
}

void
Java_org_videolan_libvlc_MediaPlayer_nativeSetVideoCallBack(JNIEnv *env,jobject thiz,jobject callback)
{
