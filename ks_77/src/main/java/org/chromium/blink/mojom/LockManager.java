package org.chromium.blink.mojom;

import org.chromium.mojo.bindings.AssociatedInterfaceNotSupported;
import org.chromium.mojo.bindings.Callbacks;
import org.chromium.mojo.bindings.DeserializationException;
import org.chromium.mojo.bindings.Interface;

/* loaded from: xwalk_main_fat-77.3.aar:classes.jar:org/chromium/blink/mojom/LockManager.class */
public interface LockManager extends Interface {
    public static final Interface.Manager<LockManager, Proxy> MANAGER = LockManager_Internal.MANAGER;

    /* loaded from: xwalk_main_fat-77.3.aar:classes.jar:org/chromium/blink/mojom/LockManager$Proxy.class */
    public interface Proxy extends LockManager, Interface.Proxy {
    }

    /* loaded from: xwalk_main_fat-77.3.aar:classes.jar:org/chromium/blink/mojom/LockManager$QueryStateResponse.class */
    public interface QueryStateResponse extends Callbacks.Callback2<LockInfo[], LockInfo[]> {
    }

    void requestLock(String str, int i, int i2, AssociatedInterfaceNotSupported associatedInterfaceNotSupported);

    void queryState(QueryStateResponse queryStateResponse);

    /* loaded from: xwalk_main_fat-77.3.aar:classes.jar:org/chromium/blink/mojom/LockManager$WaitMode.class */
    public static final class WaitMode {
        private static final boolean IS_EXTENSIBLE = false;
        public static final int WAIT = 0;
        public static final int NO_WAIT = 1;
        public static final int PREEMPT = 2;

        public static boolean isKnownValue(int value) {
            return value >= 0 && value <= 2;
        }

        public static void validate(int value) {
            if (!isKnownValue(value)) {
                throw new DeserializationException("Invalid enum value.");
            }
        }

        private WaitMode() {
        }
    }
}
