...
public class XWalkResourceClient {
     ...
     * @since 1.0
     */
    // TODO(iotto) : Maybe use it when needed!
    // public XWalkResourceClient(XWalkViewInternal view) {
    ...
    /**
     * Notify the client that initial HTML document has been completely loaded and parsed, without
     * waiting for stylesheets, images, and subframes to finish loading. This is similar to
    ...
    /**
     * Notify the client that the XWalkViewInternal will load the resource specified by the given
     * url.
     *
     * @param view the owner XWalkViewInternal instance.
    ...
    /**
     * Notify the client that the XWalkViewInternal completes to load the resource specified by the
     * given url.
     *
     * @param view the owner XWalkViewInternal instance.
    ...
    /**
     * Notify the client of a resource request and allow the client to return the data. If the
     * return value is null, the XWalkViewInternal will continue to load the resource as usual.
     * Otherwise, the return response and data will be used. NOTE: This method is called by the
     * network thread so clients should exercise caution when accessing private data.
     *
     * @param view The owner XWalkViewInternal instance that is requesting the resource.
     * @param url The raw url of the resource.
     * @return A {@link android.webkit.WebResourceResponse} containing the response information or
     *         null if the XWalkViewInternal should load the resource itself.
     * @deprecated Use {@link #shouldInterceptLoadRequest(XWalkView, XWalkWebResourceRequest)}
     *             instead.
     * @since 1.0
     */
     ...
     * @return A {@link org.xwalk.core.XWalkWebResourceResponse} containing the response information
     *         or null if the XWalkViewInternal should load the resource itself.
     * @since 6.0
     */
     ...
     * loaded in the current XWalkViewInternal. If XWalkClient is not provided, by default
     * XWalkViewInternal will ask Activity Manager to choose the proper handler for the url. If
     * XWalkClient is provided, return true means the host application handles the url, while return
     * false means the current XWalkViewInternal handles the url.
     *
     * @param view The XWalkViewInternal that is initiating the callback.
     * @param url The url to be loaded.
     * @return True if the host application wants to leave the current XWalkViewInternal and handle
     *         the url itself, otherwise return false.
     * @since 2.1
     */
     ...
     */
    public boolean onRewriteUrlIfNeeded(XWalkView view, RewriteUrlValue rewriteValue) {
        return false;
    }
     ...
     * . Note that the decision may be retained for use in response to future SSL errors. The
     * default behavior is to pop up a dialog
     *
    ...
    public void onReceivedSslError(XWalkView view, ValueCallback<Boolean> callback,
        ...
        return;
        /*
         * final ValueCallback<Boolean> valueCallback = callback; AlertDialog.Builder dialogBuilder
         * = new AlertDialog.Builder(view.getContext());
         * dialogBuilder.setTitle(R.string.ssl_alert_title) .setPositiveButton(android.R.string.ok,
         * new DialogInterface.OnClickListener() {
         * @Override public void onClick(DialogInterface dialog, int which) {
         * valueCallback.onReceiveValue(true); dialog.dismiss(); } })
         * .setNegativeButton(android.R.string.cancel, new DialogInterface.OnClickListener() {
         * @Override public void onClick(DialogInterface dialog, int which) {
         * valueCallback.onReceiveValue(false); dialog.dismiss(); } }).setOnCancelListener(new
         * DialogInterface.OnCancelListener() {
         * @Override public void onCancel(DialogInterface dialog) {
         * valueCallback.onReceiveValue(false); } }); dialogBuilder.create().show();
         */
    }
     ...
     * respond: proceed(), cancel() or ignore(). XWalkView remembers the response if proceed() or
     * cancel() is called and does not call onReceivedClientCertRequest() again for the same host
     * and port pair. XWalkView does not remember the response if ignore() is called. This method is
     * called on the UI thread. During the callback, the connection is suspended. The default
     * behavior is to cancel, returning no client certificate.
     *
     * @param view The XWalkView that is initiating the callback
    ...
    /**
     * Notify the host application that an HTTP response has been received from the server while
     * loading a resource. This callback will be called for any resource (iframe, image, etc), not
     * just for the main page. Thus, it is recommended to perform minimum required work in this
     * callback. This method behaves similarly to the Android WebView's onReceivedHttpError if the
     * HTTP response has a status code &gt;= 400. If there are no errors, {@code response} contains
     * the cookies set by the HTTP response.
     *
     * @param view The XWalkView that is initiating the callback
    ...
    /**
     * Construct an instance of XWalkWebResourceResponse for application usage with no variables
     * set. You <b>must</b> call {@link XWalkWebResourceResponse#ready} after setting data or the
     * thread will hang indefinately.
     *
    ...
    /**
     * Construct an instance of XWalkWebResourceResponse for application usage.
     *
