 ...
 */
public class XWalkCleanupReference extends WeakReference<Object> {
    private static final String TAG = "XWalkCleanupReference";

    private static final boolean DEBUG = false;  // Always check in as false!

    // The VM will enqueue XWalkCleanupReference instance onto sGcQueue when it becomes eligible for
    ...
    private static final Thread sReaperThread = new Thread(TAG) {
        ...
        public void run() {
            while (true) {
                try {
                    XWalkCleanupReference ref = (XWalkCleanupReference) sGcQueue.remove();
    ...
    // Add a new reference to sRefs. |msg.obj| is the XWalkCleanupReference to add.
    private static final int ADD_REF = 1;
    // Remove reference from sRefs. |msg.obj| is the XWalkCleanupReference to remove.
    ...
    private static class LazyHolder {
        static final Handler sHandler = new Handler(ThreadUtils.getUiThreadLooper()) {
            ...
            public void handleMessage(Message msg) {
                try {
                    TraceEvent.begin("XWalkCleanupReference.LazyHolder.handleMessage");
                    XWalkCleanupReference ref = (XWalkCleanupReference) msg.obj;
                    switch (msg.what) {
                        case ADD_REF:
                            sRefs.add(ref);
                            break;
                    ...
                    synchronized (sCleanupMonitor) {
                        ...
                        while ((ref = (XWalkCleanupReference) sGcQueue.poll()) != null) {
                            ref.runCleanupTaskInternal();
                        }
                        sCleanupMonitor.notifyAll();
                    }
                } finally {
                    TraceEvent.end("XWalkCleanupReference.LazyHolder.handleMessage");
                }
            }
        };
    }
    ...
    /**
     * Keep a strong reference to {@link XWalkCleanupReference} so that it will
     * actually get enqueued.
     * Only accessed on the UI thread.
     */
    private static Set<XWalkCleanupReference> sRefs = new HashSet<XWalkCleanupReference>();
     ...
     */
    public XWalkCleanupReference(Object obj, Runnable cleanupTask) {
        super(obj, sGcQueue);
